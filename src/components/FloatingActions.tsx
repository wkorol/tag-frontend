...
export function FloatingActions({ orderTargetId = 'vehicle-selection', hide = false }: FloatingActionsProps) {
  ...
  return (
    <>
      ...
      >
        <div ref={desktopMenuRef} className="flex flex-col items-end gap-2">
          {isOrderMenuOpen ? (
            <div id="floating-order-menu-desktop" className="mb-2 flex w-64 flex-col gap-2 animate-slideDown">
              <a
                href={`${basePath}/`}
                onClick={(event) => {
          ...
          >
            ...
            />
            <span className="relative max-w-[86px] px-4 text-center text-[11px] font-semibold leading-tight tracking-tight">
              {t.common.orderNow}
            </span>
            <svg viewBox="0 0 20 20" aria-hidden="true" className="relative mt-0.5 h-4 w-4">
              {isOrderMenuOpen ? (
                <path fill="currentColor" d="M14.7 12.7a1 1 0 01-1.4 0L10 9.42l-3.3 3.3a1 1 0 01-1.4-1.42l4-4a1 1 0 011.4 0l4 4a1 1 0 010 1.42z" />
              ) : (
                <path fill="currentColor" d="M5.3 7.3a1 1 0 011.4 0L10 10.58l3.3-3.3a1 1 0 111.4 1.42l-4 4a1 1 0 01-1.4 0l-4-4a1 1 0 010-1.42z" />
              )}
            </svg>
          </button>
        </div>
      ...
      >
        <div ref={mobileMenuRef} className="flex flex-col gap-2">
          {isOrderMenuOpen ? (
            <div id="floating-order-menu-mobile" className="grid gap-2 animate-slideDown">
              <a
                href={`${basePath}/`}
                onClick={(event) => {
                  event.preventDefault();
          ...
          >
            <span
              ...
              className="pointer-events-none absolute inset-0 rounded-full bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.18),transparent_58%)]"
            />
            <span className="relative max-w-[70px] px-3 text-center text-[10px] font-semibold leading-tight tracking-tight">
              {t.common.orderNow}
            </span>
            <svg viewBox="0 0 20 20" aria-hidden="true" className="relative mt-0.5 h-4 w-4">
              {isOrderMenuOpen ? (
